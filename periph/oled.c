 ...
 * @LastEditTime: 2023-09-02 11:18:52
...
static uint8_t LocateDateptr(uint8_t *data, uint8_t *font_width, uint8_t *font_offset, uint8_t *font_height);
static uint8_t *LocateDateptr1(uint8_t *data);
...
void oled_opr_init(void *dev_obj_opr)
{
    ...
    ((OLED_OPR_T *)dev_obj_opr)->close = close;
}
        ...
        case OLED_PRINT_CH :
        {
            ...
            while(op->data.data_ptr[m] != '\0')
			{
                ...
                switch(font)
                {
                    case 1://F6x8
                    {
                        OLED_SetPos(op->data.x, op->data.y/8);
                        for(n = 0; n < i; n++)
                        ...
                        OLED_SetPos(op->data.x, op->data.y/8+1);
                        for(n = 0; n < i; n++)
                    ...
                    case 2://F16x24
                    {
				        ...
				        OLED_SetPos(op->data.x, op->data.y/8+2);
                        for(n = 0; n < i; n++)
                        ...
                        if(!IS_DATA_SPILL(k-1, y_offset)) break;
                        OLED_SetPos(op->data.x, op->data.y/8+3);
                        for(n = 0; n < i; n++)
 ...
 * @retval: font address
 */
static uint8_t LocateDateptr(uint8_t *data, uint8_t *font_width, uint8_t *font_offset, uint8_t*font_heigth)
{
    switch((uint8_t)data)
    {
        case 0: 
